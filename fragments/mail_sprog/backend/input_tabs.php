...
<tr class="editing-tr-heading">
    ...
    <td><?= $itemId ?? '-' ?></td>
    <td colspan="2"></td>
    <td colspan="2"><input class="form-control" type="text" name="wildcard" value="<?= htmlspecialchars($wildcard) ?>" <?= $readonly ?> /></td>
    <td>
        <button class="btn btn-save btn-block" type="submit" name="func" value="save_wildcard"><?= \rex_i18n::msg('update') ?></button>
    </td>
    <td>
        <a href="<?= $list->getUrl([$pager->getCursorName() => $pager->getCursor()]) ?>" class="btn btn-primary btn-block" data-pjax="[data-list-container]">
            <?= \rex_i18n::msg('abort') ?>
        </a>
    </td>
</tr>
    ...
    <td colspan="2" class="edit-input-td">
        <div class="tab-content">
            <?php foreach ($langs as $langIdx => $lang): ?>
                ...
                ?>
                <div role="tabpanel" class="tab-pane  <?= $langIdx == 0 ? 'active' : '' ?>" id="lang-<?= $lang->getId() ?>">

                    <div class="form-group">
                        <label class="control-label"><?= rex_i18n::msg('subject') ?></label>
                        <input type="text" class="form-control" name="subject_<?= $lang->getId() ?>" value="<?= $_subject ?>">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control tiny5-editor" name="replace_<?= $lang->getId() ?>"
                            ...
                            data-tiny-mail-sprog><?= $_value ?></textarea>
                    </div>
